...
This notebook contains analyses using anvi'o version 8 [@Eren2021; @delmont2018] (development branch). For more in-depth information about anvi'o go to their official online site here: <https://anvio.org/>
...
Using `anvi-estimate-metabolism` we predicted KEGG module completeness as described in [@veseli2023] for the the organisms listed in `CorPGA_MetabolicAnalysis_GenomeList_v01.txt`. This list includes:
...
The `CorPGA_modules.txt` file generated in the previous step is used by `anvi-compute-metabolic-enrichment` to calculate KEGG module enrichment [@shaiber2020] across different groups of genomes. Enrichment was explored across different genome groupings as described in the following files:
...
We used `anvi-display-functions` to perform this functional enrichment [@shaiber2020]. It runs the enrichment for the desired annotations (in this case KOfam). It saves the output to a system temp file that was recovered, renamed and saved into the same `path_o` along with the database file.
...
As described above, metabolic enrichment was explored across different genome groupings, however only the analysis comparing the 4 nasal *Corynebacterium* as defined in `CorPGA_MetabolicAnalysis_Groups_4Cor.txt` was included in the final manuscript. Supplemental Table S4 includes the following tabs:

-   **A. Enriched_Modules_4Cor:** Corresponds to the output file from `anvi-compute-metabolic-enrichment`
-   **B. OtherEnriched_KOs_4Cor:** Corresponds to the output file `CorPGA_enriched_KOs_4Cor_Filtered.csv`. Includes only the enriched KOs that are not part of any module already described in tab A.
...
The `Assimilatory sulfate reduction, sulfate => H2S` module (M00176) had a stepwise completion score = 0.5 in most of the *Corynebacterium* genomes. However, this module was completely absent in all the *C. pseudodiphtheriticum* strains from the U.S. and 4 from Botswana. We look for KO assignments for the enzymes encoded in the *fpr2cysIXHDNYZ* operon [@Rückert2005] that are not part of the M00176 module definition. Genes annotated as K00528 (*fpr*) were identified in all the *Corynebacterium* genomes. A manual exploration of the gene annotations in the genomic region around the *fpr* in all *C. pseudodiphtheriticum* strains, allowed us to identified *hchA* as a common gene downstream the *fpr2cysIXHDNYZ* operon.

This code uses `BEDTools` [@Quinlan2010; @Quinlan2014] to extract annotations and sequences for the *fpr* to *hchA* region for all *C. pseudodiphtheriticum* genomes:
...
We use MAFFT [@Katoh2002; @Katoh2013] to align the extracted sequences.
...
We used AliView [@Larsson2014] to inspect the output file and to reverse complement the alignment to match the operon order. We saved the reversed complemented version of the alignment as `ASR_Cps_region_align_clean_R.fasta`.
...
The version of the alignment file with the reference sequence is the input for the visualization with the `msavisr` package [@vragh2022].
...
We performed pangenomic level analysis in 107 genomes: the 102 *Corynebacterium* genomes selected for metabolic analysis (Cac_ATCC_49726 & Cps_090104 were excluded for this analysis) plus the 5 *Corynebacterium* listed in **Table S1D**:
