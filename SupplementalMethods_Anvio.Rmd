...
# Calculating mean_pathwise_module_completeness and preparing plot labels 
Modules_Means = Modules %>% 
  group_by(modules_label, genome_name, .drop = FALSE) %>%
  summarise(mean_pathwise_module_completeness = mean(pathwise_module_completeness)) %>%
  mutate(mean_pathwise_module_completeness = coalesce(mean_pathwise_module_completeness, 0)) %>%
  mutate(species_name = str_extract(genome_name, "^[^_]+")) %>%
...
# Note. If using stepwise_module_completeness instead of pathwise_module_completeness some modules can become = o for all genomes and might be a good idea to remove those before creating the Modules_Means table. Code for reference only, but not used in final figure creation.
Modules_cleaned <- Modules %>%
  group_by(module) %>%
  filter(!all(stepwise_module_completeness == 0)) %>%
  mutate(across(where(is.factor), droplevels))

all_zero <- setdiff(Modules$module, Modules_cleaned$module)
all_zero
```
...
# Heatmap plot
plot <- ggplot(Modules_Means, aes(x = modules_final_label, y = styled_genome_name, fill = mean_pathwise_module_completeness)) + 
...
legend <- ggplot(Modules_Means, aes(x = module_category, y = mean_pathwise_module_completeness, fill = module_category)) +
...
Modules_MeansSummary = Modules_Means %>% 
  ...
  summarise(mean_bySpecies = mean(mean_pathwise_module_completeness))
```
