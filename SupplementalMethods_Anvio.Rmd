...
Using `anvi-estimate-metabolism` we predicted KEGG module completeness as described in [@kanehisa2011a] for the the organisms listed in `CorPGA_MetabolicAnalysis_GenomeList_v01.txt`. This list includes the 102 *Corynebacterium* genomes selected for metabolic analysis (Cac_ATCC_49726 & Cps_090104 were excluded for this analysis, as well as all *C. macginleyi* genomes), plus 27 for the 28 *Dolosigranulum pigrum* genomes described in [@floresramos2021] (KPL3086 was not included in the analysis due to being highly similar to KPL3065) as well as 9 reference genomes with their original annotations as per NCBI.
...
plot_1 <-ggplot(Modules_Distributions, aes(y = stepwise_module_completeness, x = species_name, fill = species_name)) +
  facet_wrap(~ module_category, ncol=2) +
  geom_violin() +
  scale_fill_manual(values = c("#6A329F", "#FF8C00", "#FF0000", "#3B54A4", "grey30")) +
  theme_bw()

plot_1
ggsave(plot = plot_1, filename = "data/Anvio8/Metabolic_Analysis/plots/plot1.jpg", height = 10, width = 10, device ='jpg', dpi = 500)
```
...
plot_2 <- ggplot(Modules_Distributions_Means, aes(x=genome_name, y=modules_final_label, fill=mean_stepwise_module_completeness)) + 
...
plot_2
ggsave(plot = plot_2, filename = "data/Anvio8/Metabolic_Analysis/plots/plot2.jpg", height = 10, width = 20, device ='jpg', dpi = 500)
```

```{r eval = FALSE, message=FALSE, warning=FALSE}
Modules_Distributions_Summary = Modules_Distributions %>% 
  filter(stepwise_module_completeness==1) %>%
  group_by(genome_name, module_category) %>% 
  count() %>% 
  mutate(species_name = str_extract(genome_name, "^[^_]+")) %>%
  group_by(species_name, module_category) %>%
  summarize(average = mean(n))
  
plot_3 <- ggplot(Modules_Distributions_Summary, aes(fill=module_category, y=average, x = factor(species_name, level = c("Cpr", "Cps", "Cac", "Ctu", "Dpi")))) + 
  geom_bar(position="stack", stat="identity") + 
  scale_fill_brewer(palette = "Set2") + 
  labs(x="Species", y="Average # of Complete KEGG Modules per Genome") +
  scale_y_continuous(expand=c(0, 0)) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 15, colour = c("#FF8C00","#FF0000","#6A329F","#3B54A4","grey30")),
        legend.title=element_text(size=15),
        legend.text=element_text(size=11)) +
  guides(fill = guide_legend(title="Module Category"))


plot_3
ggsave(plot = plot_3, filename = "data/Anvio8/Metabolic_Analysis/plots/plot_3.jpg", height = 5, width = 8, device ='jpg', dpi = 500)
```
...
output <- fasta_file_for_GC(Summary, "GC_00000360", output_path)
output <- fasta_file_for_GC(Summary, "GC_00000533", output_path)
output <- fasta_file_for_GC(Summary, "GC_00000144", output_path)
output <- fasta_file_for_GC(Summary, "GC_00000081", output_path)
output <- fasta_file_for_GC(Summary, "GC_00000208", output_path)
output <- fasta_file_for_GC(Summary, "GC_00001033", output_path)
