...
colorsSpecies <- c("#FF8C00", "#FF0000", "#6A329F", "#3B54A4", "grey80")
...
plot <- ggplot(Modules_Means, aes(x=modules_final_label, y=styled_genome_name, fill=mean_stepwise_module_completeness)) + 
  ...
  guides(fill = guide_colourbar(title="\nAverage\nstepwise\nmodule\ncompleteness", title.position="left", title.hjust=0.5, barwidth=0.8, barheight=3))
```
...
### Code to explore GCs of interest
...
info_for_GC <- function(Summary, output_path, GC, gene) {
  ...
  protein_id <- data$accession[data$genome_name=="Cgl_ATCC_13032"]
  KO <- data$KOfam_ACC[data$genome_name=="Cgl_ATCC_13032"]
  index <- data$combined_homogeneity_index[1]

  
  cat(protein_id[1], gene, GC, KO[1], "has", num_seqs, "aa sequences across", num_genomes, "genomes with", index ,"combined homogeneity. \n")
}
```
...
```{r}
data <- Summary %>% 
    filter(KOfam_ACC == "K01915")
unique(data$gene_cluster_id)
```
