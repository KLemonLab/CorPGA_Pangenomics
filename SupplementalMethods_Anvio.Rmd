...
#### *C. pseudodiphtheriticum* pangenome (3,588 GCs)
...
#### *C. accolens* pangenome (3,427 GCs)
...
Using `anvi-estimate-metabolism` we predicted KEGG module completeness as described in [@kanehisa2011a] for the the organisms listed in `CorPGA_MetabolicAnalysis_GenomeList_v01.txt`. This list includes the 102 *Corynebacterium* genomes selected for metabolic analysis (Cac_ATCC_49726 & Cps_090104 were excluded for these analysis, as well as all *C. macginleyi* genomes), plus 27 for the 28 *Dolosigranulum pigrum* genomes described in [@floresramos2021] (KPL3086 was not included in the analysis due to being highly similar to KPL3065) as well as 9 reference genomes with their original annotations as per NCBI.
...
plot_1 <-ggplot(Modules_Distributions, aes(y = stepwise_module_completeness, x = species_name, fill = species_name)) +
  ...
  geom_violin() +
  scale_fill_manual(values = c("darkorange", "red", "darkviolet", "navy", "grey30")) +
  theme_bw()

plot_1
ggsave(plot = plot_1, filename = "data/plots/plot1.jpg", height = 10, width = 10, device ='jpg', dpi = 500)
```
...
plot_2 <- ggplot(Modules_Distributions_Means, aes(x=genome_name, y=modules_final_label, fill=mean_stepwise_module_completeness)) + 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        legend.position="top") +
  guides(col = guide_colourbar(title="Average step module completeness"))

plot_2
ggsave(plot = plot_2, filename = "data/plots/plot2.jpg", height = 10, width = 20, device ='jpg', dpi = 500)
```
...
plot_3 <- ggplot(Modules_Distributions_Summary, aes(fill=module_category, y=average, x = factor(species_name, level = c("Cpr", "Cps", "Cac", "Ctu", "Dpi")))) + 
  ...
  theme_classic() +
  theme(axis.text.x = element_text(size = 15, colour = c("darkorange", "red", "darkviolet", "navy", "grey30")),
...
plot_3
ggsave(plot = plot_3, filename = "data/plots/plot_3.jpg", height = 5, width = 8, device ='jpg', dpi = 500)
```
