...
# Copy .gbk files to certain directories
...
```{bash, eval=FALSE}
# Make a directory to hold codon alignments
mkdir -p "analysis_GH/Codon_alignments"

# Move the codon alignments folder to the file we created
...
This will compare the two conservative core GCs of only the species of interest versus the species of interest plus an outgroup. The addition of an outgroup causes the conservative core to decrease in value due to less gene clusters being shared with the outgroup. We are able to generate a list of which gene clusters are not shared with the outgroup. We can use this list to recover the lost gene clusters back into our analysis when making phylogenies.
...
# Go inside Recovered_GCs folder
cd "Recovered_GCs_Cpr" # Go inside recovered GCs folder

# Moves all recovered GCs faa and faa files into the (Cpr + outgroup) faa and fna directory
mv *.f* "analysis_GH/Ctu_outgroup_faa_fna"
```
...
# Go inside Recovered_GCs folder
cd "Recovered_GCs_Cps" # Go inside recovered GCs folder
...
# Go inside Recovered_GCs folder
cd "Recovered_GCs_Cac" # Go inside recovered GCs folder
...
# Go inside Recovered_GCs folder
cd "Recovered_GCs_Ctu" # Go inside recovered GCs folder
...
First we located and opened the file **lib/marfil_homology.pm**, and identified lines 136-137, which control how a gene is compared to previously processed pangenome genes in order to call it novel were changed:
...
```{bash, eval=FALSE}
cd get_homologues
./get_homologues.pl -d "/analysis_GH/Prokka_out/Prokka_Cps_gbk"
 -c -n 8 -M -C 90 
```
...
```{bash, eval=FALSE}
cd get_homologues
./get_homologues.pl -d "/analysis_GH/Prokka_out/Prokka_Cac_gbk"
 -c -n 8 -M -C 90 
```
...
```{bash, eval=FALSE}
cd get_homologues
./get_homologues.pl -d "/analysis_GH/Prokka_out/Prokka_Ctu_gbk"
 -c -n 8 -M -C 90 
```
