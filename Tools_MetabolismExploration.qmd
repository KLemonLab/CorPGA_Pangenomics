...
## Explore a specific genomic region
...
```{bash}
cd data/Anvio8/Metabolic_Analysis/Pathway_Exploration/ARS

grep -E 'Name=hchA|Name=ipdC' Cps_MSK127.gff | sort -k4,4n | awk 'NR==1 {start=$4} NR==2 {end=$5} END {print $1 "\t" start "\t" end}' > Cps_MSK127_coordinates.bed
bedtools getfasta -fi Cps_MSK127.fsa -bed Cps_MSK127_coordinates.bed -fo Cps_MSK127_sequences.fasta
...
awk '/^##FASTA/{flag=1;next} {if(!flag) print}' your_file.gff > annotations_only.gff
awk '/^##FASTA/{flag=1} {if(flag) print}' your_file.gff > sequence_part.fasta
grep -v '^##FASTA' sequence_part.fasta > cleaned_sequence_part.fasta


grep -E 'Name=hchA|Name=ipdC' annotations_only.gff | sort -k4,4n | awk 'NR==1 {start=$4} NR==2 {end=$5} END {print $1 "\t" start "\t" end}' > region_coordinates.bed
bedtools intersect -a annotations_only.gff -b region_coordinates.bed -wa > annotations_in_region.gff
bedtools getfasta -fi cleaned_sequence_part.fasta -bed region_coordinates.bed -fo extracted_sequences.fasta
