...
HMP_select <- HMP %>% 
  ...
  filter(grepl('pigrum', `Species/Supraspecies`) | 
         grepl('aureus', `Species/Supraspecies`) |   
         grepl('accolens', `Species/Supraspecies`) | 
...
HMP_select <- HMP_select %>% t %>% as.data.frame() %>% row_to_names(1)
HMP_select <- HMP_select %>% mutate_if(is.character, as.numeric)
```

```{r}
# Subset with >0 reads == TRUE
HMP_select_reads <- HMP_select %>% 
  mutate(Dpi = ifelse(pigrum > 0, TRUE, FALSE)) %>% 
  mutate(Sau = ifelse(aureus > 0, TRUE, FALSE)) %>%   
  ...
  select(c("Dpi","Sau","Cac","Ctu","Cpr","Cps"))
```

### Venn Diagram
```{r}
sample_list <- list(
  #Dpi = row.names(HMP_select_reads %>% filter(Dpi == "TRUE")),
  #Sau = row.names(HMP_select_reads %>% filter(Sau == "TRUE")),
  Cpr = row.names(HMP_select_reads %>% filter(Cpr == "TRUE")),
  Cac = row.names(HMP_select_reads %>% filter(Cac == "TRUE")),
  `Ctu/Cna` = row.names(HMP_select_reads %>% filter(Ctu == "TRUE")),
  Cps = row.names(HMP_select_reads %>% filter(Cps == "TRUE"))
  )
...
p <- ggVennDiagram(sample_list,
                   ...
                   label_alpha = 0, 
                   set_size = 13,
                   #set_color = c("pink","green","#FF8C00","#9400D3","#0000ff","#FF0000")
                   #set_color = c("green","#FF8C00","#9400D3","#0000ff","#FF0000")
                   set_color = c("#FF8C00","#9400D3","#0000ff","#FF0000")
                   #set_color = c("pink","green","#9400D3","#0000ff")
)
...
ggsave(plot = p, filename = "data/other/venn_HMP.svg", width = 8, height = 6, dpi = 300)
```

### All combinations of species
```{r}
#HMP_select_reads <- HMP_select_reads %>% select(-Dpi)

# Generate combinations
combinations <- unlist(lapply(1:5, function(x) combn(names(HMP_select_reads), x, simplify = FALSE)), recursive = FALSE)
combinations_mat <- do.call(rbind, combinations)
```

```{r}
# Iterate over combinations and create new columns
Table_combinations <- HMP_select_reads
...
```{r}
HMP_select_reads$Sum <- rowSums(Filter(is.logical, HMP_select_reads))
Table_counts <- HMP_select_reads %>% 
