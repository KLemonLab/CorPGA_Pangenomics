...
plot_avg_OD600 <- ggplot(avg_data_OD, aes(x = Time, y = AvgOD, color = Species)) +
  ...
  labs(
    title = "Glycogen Bacterial Growth Curves",
    x = "Time (hours)",
    y = "Average OD600",
    color = "Species"
  ) +
...
plot_avg_OD600 # Display the plot
```
...
plot_avg_Lum_Norm <- ggplot(avg_data_Lum_Norm, aes(x = TimePoint, y = AvgLum, fill = Species)) +
  ...
  geom_errorbar(aes(ymin = AvgLum - SE, ymax = AvgLum + SE), position = position_dodge(width = 0.9), width = 0.2) + # Add error bars
  labs(
    title = "OD Normalized Glycogen Concentration",
    x = "Time (hours)",
...
plot_avg_Lum_Norm # Display the plot
```
...
plot_avg_OD600 <- ggplot(avg_data_OD, aes(x = Time, y = AvgOD, color = Species)) +
  ...
  annotate("text", x = Inf, y = Inf, label = "A", hjust = 1, vjust = 1, size = 5, fontface = "bold")
...
plot_avg_Lum_Norm <- ggplot(avg_data_Lum_Norm, aes(x = TimePoint, y = AvgLum, fill = Species)) +
  ...
  annotate("text", x = Inf, y = Inf, label = "B", hjust = 1, vjust = 1, size = 5, fontface = "bold")

# Combine the plots using ggarrange
combined_plot <- ggarrange(plot_avg_OD600, plot_avg_Lum_Norm, ncol = 2, common.legend = TRUE, legend = "right")
